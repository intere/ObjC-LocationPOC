...
@interface TrackListMetadataTracksTest : XCTestCase{
	NSString *sampleJson;
	TrackListMetadataTracks *deserialized;
}
@end
...
@implementation TrackListMetadataTracksTest

- (void)setUp
{
	[super setUp];
	NSString *pathToResource = [[NSBundle bundleForClass:self.class] pathForResource:@"track_list_metadata_tracks" ofType:@"json"];
	NSError *error = nil;
	...
	deserialized = [TrackListMetadataTracks fromDictionary:dict];
	XCTAssertNotNil(deserialized, @"The TrackListMetadataTracks failed to deserialize properly");
}
...
-(void)testVert {
	XCTAssertEqualWithAccuracy(0.5029296875, deserialized.vert, 0.01, @"Failed to properly deserialize the vert");
}
